 ...
 *
 * Contents of this class were generated by the following code:
 * {{{
 * val files = new java.io.File("./tptp/syn").listFiles.filter(_.getName.endsWith(".p"))
 *   for (f <- files) {
 *     println("test(\"" + f.getName + "\") {")
 *     println("  val source = scala.io.Source.fromFile(\"./tptp/syn/" + f.getName + "\", \"utf-8\")")
 *     println("  lazy val input = new CharArrayReader(source.toArray)")
 *     println("  val parsed = TPTP.parseFile(input)")
 *     println("  assert(parsed.isRight)")
 *     println("}")
 *   }
 * }}}
 *
