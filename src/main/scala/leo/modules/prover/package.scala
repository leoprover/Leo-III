...
package object prover {
  ...
  final def effectiveInput(input: Seq[tptp.Commons.AnnotatedFormula], state: LocalGeneralState): Seq[AnnotatedClause] = {
    ...
    if (state.negConjecture.nonEmpty) {
      ...
      val relevantAxioms = if (effectiveInput.size <= 15 || state.negConjecture.exists(cl => Clause.asTerm(cl.cl).symbols.distinct.intersect(state.signature.allUserConstants).isEmpty)) {
