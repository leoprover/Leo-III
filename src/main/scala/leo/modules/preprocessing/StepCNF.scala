...
object StepCNF extends CalculusRule {
  ...
  final def apply(vargen : leo.modules.calculus.FreshVarGen,l : Literal) : CNF = if(!l.equational){
    l.left match {
      ...
      case Forall(ty :::> t) if l.polarity => val newVar = vargen(ty); One(Literal(Term.mkTermApp(t, newVar.betaNormalize), true))
      case Forall(ty :::> t) if !l.polarity => val sko = leo.modules.calculus.skTerm(ty, vargen.existingVars); One(Literal(Term.mkTermApp(t, sko).betaNormalize, false))
      case Exists(ty :::> t) if l.polarity => val sko = leo.modules.calculus.skTerm(ty, vargen.existingVars); One(Literal(Term.mkTermApp(t, sko).betaNormalize, true))
      case Exists(ty :::> t) if !l.polarity => val newVar = vargen(ty); One(Literal(Term.mkTermApp(t, newVar.betaNormalize), false))
      case _ => None(l)
    }
  } else None(l)
