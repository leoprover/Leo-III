...
object Preprocess {
  ...
  def argumentExtraction(cs : Set[AnnotatedClause]) : Set[AnnotatedClause] = {
    ...
    while(open.nonEmpty) {
      ...
      while(it.hasNext){
        val c = it.next()
        val (cl1, terms) = ArgumentExtraction(c.cl)
  ...
  def formulaRenaming(cs : Set[AnnotatedClause]) : Set[AnnotatedClause] = {
    cs.flatMap{c =>
      val (c1, c1s) = FormulaRenaming(c.cl)
  ...
  def equalityExtraction(cs : Set[AnnotatedClause]) : Set[AnnotatedClause] = {
    cs map {c =>
      val (b1, subst1) = ReplaceAndrewsEq.canApply(c.cl)
      val cl1 = if(b1) ReplaceAndrewsEq(c.cl, subst1)._1 else c.cl
      val (b2, subst2) = ReplaceLeibnizEq.canApply(cl1)
