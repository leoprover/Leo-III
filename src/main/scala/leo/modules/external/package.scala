...
import leo.modules.output.{ToTFF, ToTHF}
...
package object external {
  ...
  final def createTHFProblem(problem: Set[Clause], flag: TPTPProblem.Flag = TPTPProblem.NONE, conjecture: Clause = null)(implicit sig: Signature): String = {
    ...
    while (problemIt.hasNext) {
      val cl = problemIt.next()
      sb.append(ToTHF.toTPTP(s"ax_$counter", cl, Role_Axiom)(sig))
      sb.append("\n")
      counter += 1
    }
    if (conjecture != null) {
      ...
      sb.append(ToTHF.toTPTP("conjecture", conjecture, Role_Conjecture)(sig))
    }
    sb.toString
  }
