  ...
  protected[modules] object FactorizationControl {
    ...
    final def factor(cl: ClauseWrapper): Set[ClauseWrapper] = {
      ...
      while (maxLitIt.hasNext) {
        // FIXME Only right of maxlit search
        val (maxLitIndex, maxLit, maxLitSide) = maxLitIt.next()
        val otherLits: Seq[Literal] = ???
        val factor = factorLit(cl, maxLitIndex, maxLit, maxLitSide, otherLits)
        res = res union factor
      }

      res
    }

    final def factorLit(cl: ClauseWrapper,
                        maxLitIndex: LiteralIndex, maxLit: Literal, maxLitSide: Side,
                        others: Seq[Literal]): Set[ClauseWrapper] = {
      var res: Set[ClauseWrapper] = Set()
