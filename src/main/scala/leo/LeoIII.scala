...
class LeoILoop extends ILoop {
  ...
  addThunk {
    intp.beQuietDuring {
      ...
      intp.addImports("leo.datastructures.tptp.Commons._")
      intp.addImports("leo.datastructures.tptp.Commons.{AnnotatedFormula=>Formula}")
    }
  }
  ...
  def programInitialization() {
    ...
    Signature.withHOL(Signature.get)
    (new NormalClauseAgent(Simplification)).register(SimpleBlackboard)
  }
